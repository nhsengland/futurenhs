...
export const useDynamicElementClassName = (config: {
    ...
    useEffect(() => {
        ...
        return () => { 
            ...
            element.current?.classList.forEach(className => element.current.classList.remove(className));
            originalElement.current?.classList.forEach(className => element.current.classList.add(className));
