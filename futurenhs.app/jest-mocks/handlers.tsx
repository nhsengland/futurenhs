...
interface GroupFolder {
    ...
    firstRegistered?: {
        atUtc: string
        by: { id: string; name: string; slug: string }
    }
}
...
const groupsEndpoint = new RegExp(
    /([A-Za-z0-9\-\"\:\/]+.+)?\/v1\/users\/[A-Za-z0-9\-\"]+\/groups(\s)?(\?[A-Za-z0-9\-\"]+.+)?$/,
    'i'
)
const groupFolderEndpoint = new RegExp(
    /([A-Za-z0-9\-\"\:\/]+.+)?\/v1\/users\/[A-Za-z0-9\-\"]+\/groups\/[A-Za-z0-9\-\"]+\/folders\/[A-Za-z0-9\-\"]+(\?[A-Za-z0-9\-\"]+.+)?$/,
    'i'
)
const groupFolderContentsEndpoint = new RegExp(
    /([A-Za-z0-9\-\"\:\/]+.+)?\/v1\/users\/[A-Za-z0-9\-\"]+\/groups\/[A-Za-z0-9\-\"]+\/folders(\/[A-Za-z0-9\-\"]+\/contents)?(\?[A-Za-z0-9\-\"]+.+)?$/,
    'i'
)
const groupFileEndpoint = new RegExp(
    /([A-Za-z0-9\-\"\:\/]+.+)?\/v1\/users\/[A-Za-z0-9\-\"]+\/groups\/[A-Za-z0-9\-\"]+\/files\/[A-Za-z0-9\-\"]+(\?[A-Za-z0-9\-\"]+.+)?$/,
    'i'
)
const groupFileDownloadEndpoint = new RegExp(
    /([A-Za-z0-9\-\"\:\/]+.+)?\/v1\/users\/[A-Za-z0-9\-\"]+\/groups\/[A-Za-z0-9\-\"]+\/files\/[A-Za-z0-9\-\"]+\/download(\?[A-Za-z0-9\-\"]+.+)?$/,
    'i'
)
...
const mockSearchResults = [
    {
        ...
        description:
            'MSW Test - An Empty Group for Visual Regression Testing. DO NOT TOUCH!!!!!',
    ...
    },
    {
        ...
        description:
            'MSW Test - Visual Regression group DO NOT TOUCH/EDIT/DELETE/JOIN/ANYTHING!',
    ...
    },
    {
        ...
        description:
            "MSW Test - <p>This bottom text box is confusing - it isn't immediately clear what comment you are replying to</p>",
    ...
    },
    {
        ...
        name: 'MSW Test - Georgie, Alice, Marianthi, it would be good to get your thoughts?',
        description:
            '<p> MSW Test - Hey, how are you all. It would be great to get your thoughts on this forum discussion?</p>',
...
const getSearchResultsHandler = ({
    ...
    rest.get('*/v1/search*', async (req, res, ctx) =>
        res(
            shouldRespond &&
                ctx.json({
                    data: !shouldReturnData
                        ? null
                        : {
 ...
 * mock getAuth
 * */
const getAuthHandler = ({ status = 200 }: { status?: number }) =>
    rest.get('*/auth/userinfo', async (req, res, ctx) =>
        res(
            ctx.json({
                Id: 'msw-admin-id',
 ...
 * mock getSiteActions
 */
const getSiteActions = ({ actions = [] }: { actions?: Array<string> }) =>
    rest.get('*/v1/users/*/actions', async (req, res, ctx) =>
        res(ctx.json(actions))
    )
...
const getGroupActions = ({
    ...
    data?: { permissions?: Array<string>; memberStatus?: string }
}) =>
    rest.get('*/v1/users/*/groups/*/actions', async (req, res, ctx) =>
        res(ctx.json(data))
    )
...
const getGroup = ({
    ...
    rest.get(groupEndpoint, async (req, res, ctx) =>
        res(
            ctx.json(
                group ?? {
                    id: 'msw-aa-group',
...
const getGroups = ({
    ...
    rest.get(groupsEndpoint, async (req, res, ctx) =>
        res(
            ctx.json({
                data: groups ?? [
                    {
                        ...
                        memberCount: 0,
                        strapLineText:
                            'The MSW Operational Planning process for 2021/22 will ',
                        themeId: null,
                    },
                ],

                firstPage:
                    '/api/v1/users/msw-admin-id/groups?isMember=False&offset=0&limit=10',
                lastPage:
                    '/api/v1/users/msw-admin-id/groups?isMember=False&offset=20&limit=10',
                limit: 10,
                nextPage:
                    '/api/v1/users/msw-admin-id/groups?isMember=False&offset=10&limit=10',
                offset: 0,
                previousPage: null,
                totalPages: 3,
                totalRecords: 22,
...
const getGroupFolder = ({
    ...
    rest.get(groupFolderEndpoint, async (req, res, ctx) =>
        res(
            ctx.json(
                groupFolder ?? {
                    id: 'fake-folder-id',
 ...
 */
const getGroupFolderContents = ({
    status = 200,
    groupFolderContents,
}: {
    status?: number
    groupFolderContents?: Array<GroupFolderContent>
}) =>
    rest.get(groupFolderContentsEndpoint, async (req, res, ctx) => {
        if (req.url.pathname.split('?')[0].split('folders').length > 0) {
            return res(
                ctx.json({
                    ...
                    limit: 10,
                    firstPage:
                        '/api/v1/users/msw-admin-id/groups/aa/folders/f86d22cf-1b0e-4d24-8120-adc500d224fb/contents?offset=0&limit=10',
                    lastPage:
                        '/api/v1/users/msw-admin-id/groups/aa/folders/f86d22cf-1b0e-4d24-8120-adc500d224fb/contents?offset=0&limit=10',
                    totalPages: 1,
                    totalRecords: 3,
                    ...
                    data: groupFolderContents ?? [
                        {
                            ...
                            name: 'MSW Folder 1',
                            description:
                                'Just a test folder in folder contents',
                            firstRegistered: {
        ...
        return res(
            ctx.json({
                ...
                limit: 10,
                firstPage:
                    'api/v1/users/mock-user/groups/aa/folders?offset=0&limit=10',
                lastPage:
                    '/api/v1/users/mock-user/groups/aa/folders?offset=0&limit=10',
                totalPages: 1,
 ...
 */
const deleteFolder = ({ status = 204 }: { status?: number }) =>
    rest.delete('*/folders/*/delete', async (req, res, ctx) =>
        res(
            ctx.json({ message: 'delete response from MSW' }),
            ctx.status(status)
        )
    )
...
const getGroupFile = ({
    ...
    rest.get(groupFileEndpoint, async (req, res, ctx) =>
        res(
            ctx.json(
                groupFile ?? {
                    id: 'msw-group-file',
 ...
 */
const getGroupFileDownload = ({ status = 200 }: { status?: number }) =>
    rest.get(groupFileDownloadEndpoint, async (req, res, ctx) => {
