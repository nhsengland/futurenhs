...
describe('withGroup hof', () => {
    ...
    const user: any = { id: 'fake-admin-id', FullName: 'Mock User 2', UserAvatar: null }
    const props: any = {};
    ...
    it('returns group data', async () => {
        ...
        expect(await mockWithGroup({ req: { user }, params: { groupId } } as any)).toHaveProperty('props.themeId');
    ...
    it('service error thrown', async () => {
        ...
        const mockWithGroup = withGroup({ props, getServerSideProps });
        const serverSideProps = await mockWithGroup({ req: { user }, params: { groupId } } as any);
