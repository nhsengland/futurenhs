...
export const ContentBlockWrapper: (props: Props) => JSX.Element = ({
    ...
    const handleSetToUpdateMode = (event: any): void => shouldEnableEnterUpdate && enterUpdateModeAction?.(blockId);
    const handleSetToReadMode = (event: any): void => shouldEnableEnterRead && enterReadModeAction?.(blockId)
    const handleCreate = (event: any): void => createAction?.(blockContentTypeId);
    const handleMovePrevious = (event: any): void => shouldEnableMovePrevious && movePreviousAction?.(blockId)
    const handleMoveNext = (event: any): void => shouldEnableMoveNext && moveNextAction?.(blockId)
    ...
    return (
        ...
        <div
            ...
            id={blockId}
            tabIndex={-1}
            className={generatedClasses.wrapper} data-content-type-id={blockContentTypeId}>
            {(mode === cprud.CREATE || mode === cprud.UPDATE) &&
                <header className={generatedClasses.header}>
                    <span className={generatedClasses.headerTitle}>{blockLabels[blockContentTypeId]}</span>
                    <div className="tablet:u-flex">
                        {mode === cprud.UPDATE

                            ? <>
                                {shouldRenderMovePrevious &&
                                    <button type="button" disabled={!shouldEnableMovePrevious} onClick={handleMovePrevious} className={generatedClasses.headerButton}>
                                        <SVGIcon name="icon-chevron-up" className={generatedClasses.headerButtonIcon} />
                                        <span>Move block up</span>
                                    </button>
                                }
                                {shouldRenderMoveNext &&
                                    <button type="button" disabled={!shouldEnableMoveNext} onClick={handleMoveNext} className={generatedClasses.headerButton}>
                                        <SVGIcon name="icon-chevron-down" className={generatedClasses.headerButtonIcon} />
                                        <span>Move block down</span>
                                    </button>
                                }
                                {!isInEditMode &&
                                    <button type="button" disabled={!shouldEnableEnterUpdate} onClick={handleSetToUpdateMode} className={generatedClasses.headerButton}>
                                        <SVGIcon name="icon-edit" className={generatedClasses.headerButtonIcon} />
                                        <span>Edit</span>
                                    </button>
                                }
                                {isInEditMode &&
                                    <button type="button" disabled={!shouldEnableEnterRead} onClick={handleSetToReadMode} className={generatedClasses.headerButton}>
                                        <SVGIcon name="icon-close" className={generatedClasses.headerButtonIcon} />
                                        <span>Finish editing</span>
                                    </button>
                                }
                                <button type="button" disabled={!shouldEnableDelete} onClick={handleDeleteRequest} className={generatedClasses.headerButton}>
                                    <SVGIcon name="icon-delete" className={generatedClasses.headerButtonIcon} />
                                    <span>Delete</span>
                                </button>
                            </>

                            : <button type="button" disabled={false} onClick={handleCreate} className={generatedClasses.headerButton}>
