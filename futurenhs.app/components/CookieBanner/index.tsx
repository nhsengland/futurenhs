...
export const CookieBanner: (props: Props) => JSX.Element = ({
    ...
    const generatedClasses: any = {
        ...
        button: classNames('c-button', 'u-mr-6', 'u-mt-6', 'c-cookie-banner_button')
    }
            ...
            <LayoutWidthContainer>
                <div className={generatedClasses.wrapper}>
                    ...
                    {body?.map((paragraph, index) => {
                        return (
                            <RichText wrapperElementType='p' bodyHtml={paragraph} key={index} />
                        )
                    })}
                    <button className={generatedClasses.button} onClick={handleAcceptCookies}>{accept}</button>
                    <button className={generatedClasses.button} onClick={handleRejectCookies}>{reject}</button>
                </div>
            </LayoutWidthContainer>
        )
