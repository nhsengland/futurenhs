...
const props: Props = {
    ...
    folderContents: [
        { id: "folder1", name: "mock folder", type: "folder", text: { body: "a simple mock folder description" } },
        {
            id: "file1-2",
            name: "mock file",
            type: "file",
            ...
            modified: "2022-4-21",
            modifiedBy: {
                id: "Mock-tester", text: { userName: "Mock tester" }
            },
            createdBy: {
                id: "Mock-owner", text: { userName: "Mock Owner" }
            },
            extension: ".xlsx"
        },
        {
...
describe('folders/folderId page', () => {
    ...
    it('renders foldersHeading instead of folder header when folderId is missing', () => {
        ...
        expect(screen.getAllByText(propsCopy.contentText.foldersHeading).length).not.toBe(0);
    ...
    it('renders view details link when it is a file', () => {
        ...
        const propsCopy = Object.assign({}, props, {
            actions: [actions.GROUPS_FOLDERS_EDIT, actions.GROUPS_FILES_DELETE, actions.GROUPS_FOLDERS_DELETE, actions.GROUPS_FILES_EDIT],
            folderContents: [{
                id: "file-test",
    ...
    it('renders folder body text when available', () => {
        ...
        const propsCopy = Object.assign({}, props, { folder: { text: { body: `<span>${text}</span>` } } });
    ...
    it('renders noFolders message', () => {
        ...
        const propsCopy = Object.assign({}, props, { folderId: null, folderContents: [] });

        render(<GroupFolderContentsTemplate {...propsCopy} />);

        expect(screen.getAllByText(propsCopy.contentText.noFolders).length).toBe(1);

    });

    it('renders createFolder when Add Folders action is present', () => {

        const propsCopy = Object.assign({}, props, { actions: [actions.GROUPS_FOLDERS_ADD] });

        render(<GroupFolderContentsTemplate {...propsCopy} />);

        expect(screen.getAllByText(propsCopy.contentText.createFolder).length).toBe(1);
    ...
    it('does not render createFolder when missing Add Folders action', () => {
        ...
        expect(screen.queryAllByText(propsCopy.contentText.createFolder).length).toBe(0);

    });

    it('renders createFile when Add Files action is present', () => {

        const propsCopy = Object.assign({}, props, { actions: [actions.GROUPS_FILES_ADD] });

        render(<GroupFolderContentsTemplate {...propsCopy} />);

        expect(screen.getAllByText(propsCopy.contentText.createFile).length).toBe(1);
    ...
    it('does not render createFile when missing Add Files action', () => {
        ...
        expect(screen.queryAllByText(propsCopy.contentText.createFile).length).toBe(0);

    });

    it('gets required server side props', async () => {

        const serverSideProps = await getServerSideProps({ req: { cookies: "fake-cookie" }, params: { groupId: "mock-group-id", folderId: "mock-folder-id" } } as any);
    ...
    it('returns notFound if user does not have admin view permission', async () => {
        ...
        const serverSideProps = await getServerSideProps({ req: { cookies: "fake-cookie-101" }, params: { groupId: "mock-group-id", folderId: "mock-folder-id" } } as any);
