...
describe('Topic post', () => {
    ...
    it("Show more posts", () => {
         ...
         const localjQueryAjaxMockedFn = jest.fn(({ success }) => {
            
            const injectedDataHtml = `<ul><li class="morePosts">More posts...</li><li class="morePosts">More posts...</li></ul>`
       
            success(injectedDataHtml);
        ...
        const showMorePosts = <HTMLAnchorElement>document.getElementById('showMorePosts');
        
        showMorePosts.click();
        //test results dropped for passing the depricated show more function
        expect(jQueryShow).toBeCalledTimes(0);
        expect(jQueryHide).toBeCalledTimes(0);
        
        expect(document.getElementsByClassName('morePosts').length).toBe(0);
